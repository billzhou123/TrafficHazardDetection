...
StopSignThread::StopSignThread()
{

}

void StopSignThread::process()
{
	while (1)
	{
		if (globalStop)
		...
		//imwrite("C:\\Users\\qq3\\Desktop\\canny.jpg", signImg);

		//Image Transform
		Mat redThresh,canny;
		THDUtil::colorDetector(signImg, redThresh, Scalar(122, 45, 51), Scalar(182, 204, 255));
		pyrDown(redThresh, canny, Size(redThresh.cols / 2, redThresh.rows / 2));
		Canny(canny, canny, 30, 30 * 3, 3);

		int flag = 0;
		HoughCircles(canny, circles, CV_HOUGH_GRADIENT, 1, redThresh.rows / 8, 20, 9, 1, 10);
		for (size_t i = 0; i < circles.size(); i++)
		{
			...
			globalApproxStopSigns = circles;
			mutex_globalApproxStopSigns.unlock();
			flag = 1;

			if (THDUtil::inVicinity(center, Point(458, 95), 5) || THDUtil::inVicinity(center, Point(456, 75), 5) 
				|| THDUtil::inVicinity(center, Point(437, 83), 5) || THDUtil::inVicinity(center, Point(432, 110), 5) 
				|| THDUtil::inVicinity(center, Point(537, 65), 5))
			{
				mutex_globalDetectedStopSign.lock();
				globalDetectedStopSign = circles[i];
				mutex_globalDetectedStopSign.unlock();
			}else{
				Vec3f empty;
				empty[0] = 0;
				mutex_globalDetectedStopSign.lock();
				globalDetectedStopSign = empty;
				mutex_globalDetectedStopSign.unlock();
			}
		}
		if (flag == 0){
			mutex_globalApproxStopSigns.lock();
			globalApproxStopSigns.clear();
			mutex_globalApproxStopSigns.unlock();
		}
